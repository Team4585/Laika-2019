...
import org.huskyrobotics.frc2019.subsystems.drive.Drivetrain;   //using Drivetrain class to move motors
...
public class VisionController
{
	...
	{
        updateInputs();
		double error = Math.abs(this.x / 27); //error is x / 27. x is measured in degrees, where the max x is 27. We get a value from -1 to 1 to scale for speed output
		if(error < errorThreshold)   //checking if the error is within a threshold to stop the robot from moving
		{
    ...
    public void follow(double kp, double ki, double kd, double errorThreshold, double defaultSpeed, double targetArea)    //method for robot to follow a target while maintaing center point
	{
		...
		if( error < errorThreshold ) //if error is in between the threshold execute following statements
		{
